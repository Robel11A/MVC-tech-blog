...
router.get("/dashboard", auth, async (req, res) => {
    try {
        ...
        const posters = userData.posters ? userData.posters.map(post => post.get({ plain: true })) : [];
        res.render("dashboard", { posters, loggedIn: req.session.loggedIn, isDashboard: true });
    } catch (error) {
        console.error("ERROR occurs while fetching data from /dashboard\n", error);
        res.status(500).json({ message: "Internal error occurs, please try again later" });
    }
});
...
router.get("/post/:id", auth, async (req, res) => {
    try {
        ...
        console.error("ERROR occurs while fetching data from /post/:id\n", error);
        res.status(500).json({ message: "Internal error occurs, please try again later" });
    }
});
